...
@objc public protocol ENSideMenuProtocol {
    ...
    func popToMainView()
    func setContentViewController(contentViewController: UIViewController?, shouldPopOnce: Bool)
}
...
public class ENSideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    public func resetMenuSelectionForRow(row:Int){
        let indexPath = NSIndexPath(forRow: row, inSection: 0)
        menuTableViewController.tableView.selectRowAtIndexPath(indexPath, animated: false, scrollPosition: UITableViewScrollPosition.Middle)
    }
