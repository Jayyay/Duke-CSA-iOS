    ...
    @IBAction func onClickLike(sender: AnyObject) {
        if childDis.IdidLike {
            ...
            childDis.PFInstance.saveInBackgroundWithBlock({ (success:Bool, error:NSError?) -> Void in
                if success {
                    //push notif
                    AppNotif.pushNotification(forType: AppNotif.NotifType.NEW_EVENT_LIKE, withMessage: message, toUser: sendToUser, withSoundName: AppConstants.SoundFile.NOTIF_1, PFInstanceID: self.childDis.parent.objectId!)
                }
            })
    ...
    func onPostReply(txt:String) {
        ...
        childDis.PFInstance.saveInBackgroundWithBlock { (success:Bool, error:NSError?) -> Void in
            ...
            if success{
                ...
                self.parentVC.tableView.reloadData()
                AppNotif.pushNotification(forType: AppNotif.NotifType.NEW_EVENT_REPLY, withMessage: message, toUser: sendToUser, withSoundName: AppConstants.SoundFile.NOTIF_1, PFInstanceID: self.childDis.parent.objectId!)
            }else{
